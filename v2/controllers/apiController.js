...
class ApiController {
    ...
    // GET /api/sources - Buscar todas as fontes (agora retorna csv_files)
            ...
            try {
                ...
                const result = await client.query(`
                    SELECT 
                        id,
                        filename as nome,
                        created_at as data_upload,
                        row_count as quantidade_itens,
                        status,
                        original_structure
                    FROM sources
                ...
                const sources = result.rows.map(row => ({
                    ...
                    status: row.status,
                    colunas: Array.isArray(row.original_structure) ? row.original_structure.join(', ') : (typeof row.original_structure === 'string' ? row.original_structure : '-')
                }));
    ...
    static async createDisparo(req, res) {
        ...
        try {
            ...
            const queueResult = await client.query(`
                INSERT INTO queue (nome, source_id, configuracao, data_disparo)
                VALUES ($1, $2, $3, CURRENT_TIMESTAMP)
                RETURNING *
            `, [nome, source_id, JSON.stringify(configuracao || {})]);
            ...
            const insertPromises = solicitacoes_ids.map(solicitacao_id =>
                client.query(`
                    INSERT INTO queue_content (queue_id, solicitacao_id)
                    VALUES ($1, $2)
            ...
            console.log(`🔄 Atualizando status de ${solicitacoes_ids.length} solicitações para 'pending'`);
            await client.query(`
            ...
            console.log(`✅ Status das solicitações atualizado para 'pending' na queue: ${queue.id}`);
    ...
    static async getDisparos(req, res) {
        ...
        try {
            ...
            let query = `
                SELECT 
                    q.*,
                    s.filename as source_nome,
                    COUNT(qc.solicitacao_id) as total_solicitacoes_vinculadas,
                    COUNT(CASE WHEN qc.status = 'enviado' THEN 1 END) as envios_sucesso,
                    COUNT(CASE WHEN qc.status = 'erro' THEN 1 END) as envios_erro
                FROM queue q
    ...
    static async updateDisparo(req, res) {
        ...
        try {
            ...
            if (['executing', 'done'].includes(checkResult.rows[0].status)) {
                return res.status(400).json({
                    status: 'error',
            ...
            await client.query('DELETE FROM queue_content WHERE queue_id = $1', [id]);
            ...
            await client.query(`
                ...
                WHERE status = 'em_fila' AND id IN (
                    SELECT solicitacao_id FROM queue_content WHERE queue_id = $1
                )
            `, [id]);
            ...
            for (let i = 0; i < solicitacoes_ids.length; i++) {
                ...
                await client.query(`
                    INSERT INTO queue_content (queue_id, solicitacao_id, ordem, status)
    ...
    static async updateDisparoStatus(req, res) {
        ...
        try {
            ...
            // Validar status
            const validStatuses = ['executing', 'paused', 'done', 'cancelled', 'error', 'pending'];
    ...
    static async updateTipoMensagem(req, res) {
        ...
        try {
            ...
            const validTipos = ['1_notificacao', '2_notificacao', '3_notificacao', '4_notificacao', '5_notificacao'];
            ...
            const query = `
            UPDATE queue
            SET configuracao = jsonb_set(configuracao, '{tipo_mensagem}', to_jsonb($2::text), true)
            WHERE id = $1
            RETURNING id, nome, configuracao;
            `;
    ...
    // GET /api/sources/:id/results - Buscar resultados por source
    static async getResultsBySource(req, res) {
        const db = require('../config/database');

        try {
            console.log('🔍 getResultsBySource chamado para source:', req.params.id);
            ...
            // Buscar registros da source (tabela extraction_results)
            const query = `
            SELECT 
                er.id,
                er.status,
        ...
        `;
            ...
            // Mapear mantendo estrutura mínima + JSONB completo
            const results = result.rows.map(row => ({
                ...
                // JSONB completo e dinâmico para o frontend processar
                data: row.data || {}
            }));
            ...
            console.error('❌ Erro ao buscar resultados por source:', error);
            res.status(500).json({
                status: 'error',
                message: 'Erro interno do servidor',
                error: process.env.NODE_ENV === 'development' ? error.message : undefined
    ...
    static async executeDisparo(req, res) {
        ...
        const { mensagem_personalizada } = req.body;

        try {
            // Validar a mensagem personalizada
            if (!mensagem_personalizada || mensagem_personalizada.trim().length === 0) {
                return res.status(400).json({
                    status: 'error',
            ...
            // Atualizar a queue com a mensagem personalizada
            await client.query(`
                UPDATE queue 
                ...
                WHERE id = $1
            `, [id, JSON.stringify(mensagem_personalizada)]);
            ...
            const solicitacoesResult = await client.query(`
                ...
                FROM queue_content qc
                INNER JOIN extraction_results sol ON qc.solicitacao_id = sol.id
                WHERE qc.queue_id = $1 AND qc.status = 'pending'
                ORDER BY qc.created_at
            `, [id]);

            console.log(`📋 Solicitações pendentes encontradas: ${solicitacoesResult}`);
            ...
            if (solicitacoesResult.rows.length === 0) {
                ...
                await client.query(`
                    ...
                    WHERE id = $1
                `, [id]);
                console.log('⚠️ Queue não possui solicitações pendentes, marcada como concluída.', { texto: solicitacoesResult.rows.length });
                return res.json({
                    status: 'success',
            ...
            // Validar se todas as variáveis na mensagem personalizada existem nos dados
            const variaveis = mensagem_personalizada.match(/\{([^}]+)\}/g);
            if (variaveis) {
                const primeiroRegistro = solicitacoesResult.rows[0];
                if (primeiroRegistro && primeiroRegistro.data) {
                    const colunasDisponiveis = Object.keys(primeiroRegistro.data);
                    ...
                    if (variaveisFaltando.length > 0) {
                        return res.status(400).json({
                            status: 'error',
                            message: `As seguintes variáveis não existem nos dados: ${variaveisFaltando.join(', ')}`
                        });
                    }
                }
            }
    ...
    static async saveTypebotConfig(req, res) {
        try {
            ...
            // Mensagens livres: não validar template JSON nem fazer parsing
    ...
    static async deleteSource(req, res) {
        ...
        try {
            ...
            const checkResult = await client.query('SELECT id FROM sources WHERE id = $1', [id]);
            ...
            const disparosEmExecucao = await client.query(`
                SELECT COUNT(*) as total 
                FROM queue q
                JOIN queue_content qc ON q.id = qc.queue_id
                JOIN extraction_results s ON qc.solicitacao_id = s.id
                WHERE s.source_id = $1 AND q.status = 'executing'
            `, [id]);
    ...
    static async uploadCsv(req, res) {
        try {
            ...
            // Possíveis endpoints do serviço de extração de CSV (ordem de preferência)
            const csvServiceUrls = [];
            if (process.env.CSV_EXTRACTOR_URL) csvServiceUrls.push(process.env.CSV_EXTRACTOR_URL.replace(/\/+$/, ''));
            if (process.env.CSV_PROCESSOR_URL) csvServiceUrls.push(process.env.CSV_PROCESSOR_URL.replace(/\/+$/, ''));
            // Endereço padrão para deploy em compose
            csvServiceUrls.push('http://csv-extractor:5002');
            // Fallback útil em ambientes Windows onde o serviço roda no host
            csvServiceUrls.push('http://host.docker.internal:5002');

            // Remove duplicatas preservando ordem
            const seen = new Set();
            const triedUrls = csvServiceUrls.filter(u => {
                if (!u) return false;
            ...
            console.log('📤 Enviando para CSV extractor... Tentativas:', triedUrls);

            let lastError = null;
            let successData = null;

            for (const baseUrl of triedUrls) {
                const target = `${baseUrl}/extract-csv`;
                try {
                    console.log(`🔎 Tentando ${target}`);
                    ...
                    if (!response.ok) {
                        ...
                        console.log(`❌ Erro do CSV extractor em ${target}:`, errorData || response.status);
                        lastError = new Error(`HTTP ${response.status} from ${target}: ${errorData || 'no body'}`);
                        continue; // tentar próximo
                    }

                    successData = await response.json();
                    console.log('✅ Upload de CSV via proxy concluído com sucesso por', target);
                    break;
                } catch (err) {
                    console.warn(`⚠️ Falha ao conectar em ${target}:`, err.message || err);
                    lastError = err;
            ...
            if (!successData) {
                ...
                console.error('❌ Todas as tentativas falharam. URLs tentadas:', attempted);
                throw new Error(lastError ? lastError.message : `Não foi possível conectar ao serviço CSV. URLs tentadas: ${attempted}`);
            }

            res.json(successData);

        } catch (error) {
            console.error('❌ Erro ao fazer upload de CSV via proxy:', error.message);
            res.status(500).json({
                status: 'error',
    ...
    // GET /api/templates-message - lista todos os templates
    static async getTemplateMessages(req, res) {
        const db = require('../config/database');
        try {
            const rows = await db.findAll('template_messages', 'ORDER BY id DESC');
            res.json({ status: 'success', data: rows });
        } catch (error) {
            console.error('Erro ao listar templates de mensagem:', error);
            res.status(500).json({ status: 'error', message: 'Erro ao buscar templates', error: process.env.NODE_ENV === 'development' ? error.message : undefined });
        }
    }

    // POST /api/templates-message - cria um novo template
    static async createTemplateMessage(req, res) {
        const db = require('../config/database');
        try {
            // Validação
            const { name, description, structure } = req.body;
            if (!name || !description || !structure) {
                return res.status(400).json({ status: 'error', message: 'Todos os campos são obrigatórios.' });
            }
            // Validar estrutura JSON
            if (typeof structure !== 'object') {
                return res.status(400).json({ status: 'error', message: 'Estrutura deve ser um objeto JSON.' });
            }
            const newTemplate = await db.create('template_messages', {
                name,
                description,
                structure
            });
            res.json({ status: 'success', data: newTemplate });
        } catch (error) {
            console.error('Erro ao criar template de mensagem:', error);
            res.status(500).json({ status: 'error', message: 'Erro ao criar template', error: process.env.NODE_ENV === 'development' ? error.message : undefined });
        }
    }
}
