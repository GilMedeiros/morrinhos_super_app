...
class TypebotAPI {
     ...
     */
    // Agora sempre envia apenas a mensagem personalizada livre no payload
    processPayloadTemplate(variables) {
        console.log('DEBUG typebot.js > processPayloadTemplate: variables:', variables);
        // Sempre retorna exatamente o que chegou em variables.message, caso seja string
        return { message: typeof variables.message === 'string' ? variables.message : '' };
    }
    ...
    async sendMessage(remoteJid, variables) {
        try {
            // Recarrega dinamicamente a config antes de cada envio
            await this.loadConfig();
            console.log(`📤 [WEBHOOK] Enviando mensagem via Webhook para ${remoteJid}`);
            console.log('🔧 [WEBHOOK] Usando URL:', this.config.baseURL);

            // Processar payload usando template configurável
            // Extrai nome e telefone das variáveis (sempre presentes)
            const nome = variables.name || (variables.data && variables.data.nome) || '';
            const telefone = variables.phone || (variables.data && (variables.data.telefone || variables.data.phone)) || '';
