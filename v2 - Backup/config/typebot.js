...
class TypebotAPI {
    ...
    processPayloadTemplate(variables) {
        try {
            ...
            console.log('🔧 Template sendo usado:', template);
            console.log('🔧 Variáveis recebidas:', JSON.stringify(variables, null, 2));

            // Se não há template, usar formato padrão
            if (!template) {
                console.log('🔧 Usando formato padrão (sem template)');
                const defaultPayload = {
                    ...
                    ccp: variables.ccp || '',
                    processo: variables.processo || '',
                    valor_devido: variables.valor_devido || '',
                    tipo_mensagem: variables.tipo_mensagem || '',
                ...
                console.log('🔧 Payload padrão gerado:', JSON.stringify(defaultPayload, null, 2));
            ...
            console.log('🔧 Iniciando substituições no template...');
            console.log('🔧 Template antes das substituições:', processedTemplate);
            ...
            Object.keys(replacements).forEach(key => {
                ...
                console.log(`🔧 Processando variável ${key}: "${value}"`);
                ...
                if (processedTemplate.includes(`"{${key}}"`)) {
                    console.log(`🔧 Encontrada variável ${key} com aspas no template`);
                ...
                } else if (processedTemplate.includes(`{${key}}`)) {
                    console.log(`🔧 Encontrada variável ${key} sem aspas no template`);
                    ...
                    console.log(`🔧 ⚠️ Variável ${key} NÃO ENCONTRADA no template!`);
                }
            });

            console.log('🔧 Template após todas as substituições:', processedTemplate);
            const finalPayload = JSON.parse(processedTemplate);
            console.log('🔧 Payload final parseado:', JSON.stringify(finalPayload, null, 2));
    ...
    formatDate(date) {
        ...
        // Removendo timeZone para evitar conversão dupla
            ...
            if (payloadTemplate) {
                try {
                    const processedTemplate = payloadTemplate
                        ...
                        .replace(/\{ccp\}/g, variables.ccp || '')
                        .replace(/\{processo\}/g, variables.processo || '')
                        .replace(/\{valor_devido\}/g, variables.valor_devido || '')
