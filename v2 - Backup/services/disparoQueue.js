...
class DisparoQueue {
        ...
        try {
            ...
            const query = `
                SELECT 
                    ...
                    q.tipo_mensagem as tipo_mensagem
                FROM queue_content qc
                INNER JOIN extraction_results sol ON qc.solicitacao_id = sol.id
                INNER JOIN queue q ON qc.disparo_id = q.id
                WHERE qc.status = 'pendente' 
                AND q.status = 'executando'
                AND qc.tentativas < $1
    ...
    async processMessage(client, messageData) {
        ...
        console.log(`📤 Processando mensagem para ${contribuinte} (${celular})`);
        ...
        try {
            ...
            const variables = {
                name: contribuinte || 'Contribuinte',
                phone: formattedPhone,
                ccp: ccp || 'CCP não informado',
                processo: processo || 'Processo não informado',
                valor_devido: valor_devido || 'Valor não informado',
                tipo_mensagem: tipo_mensagem || 'Não especificado',
            ...
            if (result.success) {
                ...
                console.log(`✅ Mensagem enviada com sucesso para ${contribuinte}`);
                ...
                if (newTentativas >= this.config.maxRetries) {
                    ...
                    console.error(`❌ Falha definitiva para ${contribuinte} após ${this.config.maxRetries} tentativas`);
                } else {
                    ...
                    console.warn(`⚠️ Falha temporária para ${contribuinte} (tentativa ${newTentativas}/${this.config.maxRetries})`);
                }
            }

        } catch (error) {
            console.error(`❌ Erro ao processar mensagem para ${contribuinte}:`, error);
    ...
    async checkDisparoStatus() {
        ...
        try {
            ...
            const queuesEmExecucao = await client.query(`
                SELECT id, nome FROM queue WHERE status = 'executando'
            `);
        ...
        try {
            const stats = await client.query(`
                SELECT 
                    ...
                    COUNT(CASE WHEN qc.status = 'pendente' THEN 1 END) as pendentes,
                ...
                WHERE q.status IN ('executando', 'criado')
